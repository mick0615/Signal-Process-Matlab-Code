% M11205314-HW2-1_Code
clc; clear; close all;
set(0, 'DefaultAxesFontName', 'times');
set(0,'DefaultAxesFontSize',10);
data = xlsread('3-story steel frame - A26X (p1-test4, test5).xlsx');
timeAxis = data(:,1); %time data
data4 = data(:,2); %time response data
...
for isig = 1:length(wsigmaList)
    ...
    for I = 1:Ndata
        ...
        if idx1 <= 0
            ...
            wdata = [flipud(data4(1:(wNdata-idx2))); data4(1:idx2)];
            wdata = wfun.*wdata;
        elseif idx1 > (Ndata-wNdata+1)
            % symmetry on the edge
            Nflip = wNdata - (Ndata - idx1 + 1);
            wdata = [data4(idx1:end); flipud(data4((Ndata-Nflip+1):end))];
            wdata = wfun.*wdata;
        else
            % normal procedure
            wdata = wfun.*data4(idx1:idx2);
        end
...
plot(timeAxis,data4); title('Response Data Signal'); xlabel('Time (sec)'); ylabel('Response (g)'); grid on; grid minor;
